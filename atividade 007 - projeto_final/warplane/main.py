...
MAX_BULLETS = 5
BULLET_SPEED = 8
BULLET_RANGE = 250  # Balas desaparecem após essa distância
PLAYER_SPEED = 3
ROTATE_SPEED = 3
HITS_TO_WIN = 3
SOUND_ENABLED = True
...
pygame.display.set_caption("Ship_game")
...
# Images
plane1_img = load_image_scaled("ship/assets/airplane_black.png", PLANE_SCALE)
plane2_img = load_image_scaled("ship/assets/airplane_red.png", PLANE_SCALE)
...
class Bullet:
    ...
    def update(self):
        ...
        if self.travel >= self.max_range:
            self.alive = False  # Desaparece após distância máxima
...
# Clouds (com movimento)
clouds: list[tuple] = []
cloud_dx = 0.3  # velocidade horizontal
cloud_direction = 1  # 1 = direita, -1 = esquerda


def add_big_cloud():
    """Cria uma nuvem grande centralizada no topo, sem cobrir o meio da tela."""
    if cloud_img:
        big_cloud = pygame.transform.scale(
            cloud_img, (int(WIDTH * 0.9), int(HEIGHT * 0.25))
        )
        rect = big_cloud.get_rect(center=(WIDTH // 2, HEIGHT * 0.20))
        clouds.append(("img", big_cloud, rect))
    else:
        rect = pygame.Rect(WIDTH * 0.05, HEIGHT * 0.1, WIDTH * 0.9, HEIGHT * 0.2)
        clouds.append(("fallback", rect))


def update_clouds():
    """Faz a nuvem se mover levemente de um lado para o outro."""
    global cloud_direction
    for i, (kind, *rest) in enumerate(clouds):
        if kind == "img":
            img, rect = rest
            rect.x += cloud_dx * cloud_direction
            if rect.left <= 0 or rect.right >= WIDTH:
                cloud_direction *= -1
            clouds[i] = ("img", img, rect)
        else:
            (rect,) = rest
            rect.x += cloud_dx * cloud_direction
            if rect.left <= 0 or rect.right >= WIDTH:
                cloud_direction *= -1
            clouds[i] = ("fallback", rect)


def draw_clouds(surface: pygame.Surface):
    """Desenha a nuvem no topo da tela."""
    for kind, *rest in clouds:
        if kind == "img":
            ...
            inner = rect.inflate(-rect.w * 0.1, -rect.h * 0.2)
            pygame.draw.ellipse(surface, (235, 235, 235), inner)


add_big_cloud()  # Adiciona a nuvem
...
round_mgr = RoundManager(RoundRules(hits_to_win=HITS_TO_WIN, score_cap=99), players_by_id)
...
def update(dt: float, keys):
    update_clouds()  # atualiza o movimento da nuvem
...
# Draw
def draw():
    screen.fill((135, 206, 250))  # Fundo azul limpo
